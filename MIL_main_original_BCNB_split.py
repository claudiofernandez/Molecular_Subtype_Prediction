...
def main_cv(args):
    ...
    if "Patch_GCN" in aggregation:
        run_name = "PM_" + str(pred_mode) + "_AGGR_" + str(args.aggregation) + "_ML_" + str(
            ...
            knn) + "_GCNL_" + str(args.num_gcn_layers) + "_EF_" + str(include_edge_features)
    else:
        run_name = "PM_" + str(pred_mode) + "_AGGR_" + str(args.aggregation) + "_ML_" + str(
            magnification_level) + "_NN_bb_" + str(args.network_backbone) + "_FBB_" + str(
            args.freeze_bb_weights) + "_PS_" + str(patch_size) + "_DA_" + str(data_augmentation) + "_SN_" + str(
            ...
            ".", "") + "_Order_" + str(ordered) + "_Optim_" + str(args.optimizer_type) + "_N_" + str(
            max_instances) + "_BDG_" + str(balanced_train_datagen) + "_OWD_" + str(
            args.optimizer_weight_decay) + "_TP_" + str(tissue_percentages_max)

    # Prepare output directories
    dir_out_gnrl = os.path.join(dir_results, args.experiment_name)
    dir_out_main = os.path.join(dir_results, args.experiment_name)
    dir_out = os.path.join(dir_out_main, run_name)
    #dir_out = os.path.join(dir_out_main)
...
experiment_name = "[03_07_2023] MolSub 10x 3CLF BCNB Split"
...
for network_backbone in network_backbones:
    for aggregation in aggregations:
        for learning_rate in learning_rates:
            ...
            parser.add_argument("--dir_cv_dataset_splitting_path", default="../data/dataset-splitting",
                                type=str)  # Directory where the folds of the new splits are stored
            ...
            parser.add_argument("--magnification_level", default="10x", type=str)  # # [BCNB] 5x, 10x, 20x
            ...
            parser.add_argument('--include_edge_features', default=False,
                                help='Include edge_features (euclidean dist) in  the graph')
            parser.add_argument('--num_gcn_layers', type=int, default=5, help='# of GCN layers to use.')
            parser.add_argument('--node_knn', type=int, default=knn,
                                help='# of K nearest neighbours for graph creation.')
            parser.add_argument('--node_feature_extractor', type=str, default=node_feature_extractor,
                                help="Feature extractor that will be used for creating the nodes of the graph.")  # resnet50_3blocks_1024
            parser.add_argument('--edge_agg', type=str, default='spatial',
                                help="What edge relationship to use for aggregation.")  # It is possible to use spatial or latent edge aggregation
