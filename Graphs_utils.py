...
class WSI2Graph_Generator():
    ...
    def __init__(self, pred_column, regions_filled, bag_id, pred_mode, patch_size, data_augmentation, stain_normalization, max_instances, images_on_ram, include_background,
        ...
        if self.pred_mode == "LUMINALAvsLAUMINALBvsHER2vsTNBC": self.classes = ['Luminal A', 'Luminal B', 'HER2(+)', 'Triple negative']
        elif self.pred_mode == "LUMINALSvsHER2vsTNBC": self.classes = ['Luminal', 'HER2(+)', 'Triple negative']
        elif self.pred_mode == "OTHERvsTNBC": self.classes = ['Other', 'Triple negative']
        ...
        if self.magnification_level == "20x": self.input_shape = (3, 512, 512)
        elif self.magnification_level == "10x": self.input_shape = (3, 256, 256)
        elif self.magnification_level == "5x": self.input_shape = (3, 128, 128)
        ...
        # Generate graphs
        self.create_graphs(knn_list=self.knn_list, data_generator=self.data_generator_train, dir_results_save_graph=self.dir_results_save_graph, feature_extractor_name=self.feature_extractor_name, feature_extractor_path=self.feature_extractor_path, include_edge_features=self.include_edge_features)
        self.create_graphs(knn_list=self.knn_list, data_generator=self.data_generator_val, dir_results_save_graph=self.dir_results_save_graph, feature_extractor_name=self.feature_extractor_name, feature_extractor_path=self.feature_extractor_path, include_edge_features=self.include_edge_features)
        self.create_graphs(knn_list=self.knn_list, data_generator=self.data_generator_test, dir_results_save_graph=self.dir_results_save_graph, feature_extractor_name=self.feature_extractor_name, feature_extractor_path=self.feature_extractor_path, include_edge_features=self.include_edge_features)
    ...
    def get_MIL_datasets_and_dataloaders_coordinates(self):
        ...
        dataset_train = MILDataset_w_class_perc_coords(dir_images=self.dir_images, data_frame=self.train_ids_df, classes=self.classes,
        ...
        # Validation
        dataset_val = MILDataset_w_class_perc_coords(dir_images=self.dir_images, data_frame=self.val_ids_df, classes=self.classes,
                                                     pred_column=self.pred_column, pred_mode=self.pred_mode,
                                                     magnification_level=self.magnification_level,
                                                     bag_id=self.bag_id, input_shape=self.input_shape,
                                                     data_augmentation=self.data_augmentation, stain_normalization=self.stain_normalization,
                                                     images_on_ram=self.images_on_ram, include_background=self.include_background,
        ...
        # Test
        dataset_test = MILDataset_w_class_perc_coords(dir_images=self.dir_images, data_frame=self.test_ids_df, classes=self.classes,
                                                      pred_column=self.pred_column, pred_mode=self.pred_mode,
                                                      magnification_level=self.magnification_level,
                                                      bag_id=self.bag_id, input_shape=self.input_shape,
                                                      data_augmentation=self.data_augmentation, stain_normalization=self.stain_normalization,
        ...
        data_generator_test = MILDataGenerator_coords_rID(dataset_test, batch_size=1, shuffle=False,
                                                      ...
                                                      num_workers=self.num_workers, pred_column=self.pred_column, pred_mode=self.pred_mode,
                                                      images_on_ram=self.images_on_ram, return_patient_id=True)
            ...
            for i_iteration, (patient_id, X, Y, X_augm, img_coords) in enumerate(tqdm(data_generator, leave=True, position=0)):
                ...
                if not os.path.isfile(os.path.join(dir_results_save_graph, graph_savename)):
                    ...
                    if include_edge_features:
                        graph_creator = imgs2graph_w_edgefeatures_norm(backbone=feature_extractor_name, pretrained=True, knn=k,
